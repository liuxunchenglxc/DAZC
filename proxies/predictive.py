...
def find_measures_arrays(net_orig, trainloader, dataload_info, device, zc_imgs=None, zc_labels=None, teacher_output=None, measure_names=None, loss_fn=F.cross_entropy):
    ...
    # cache empty is too slow
    # torch.cuda.empty_cache()
    # net_orig = net_orig.cpu() 
    # torch.cuda.empty_cache()

    #given 1 minibatch of data
    if dataload == 'kd': # KD based method
        inputs, targets = zc_imgs.to(device), teacher_output.to(device)
    elif dataload == 'zc_imgs': # real meaningful samples as inputs with real labels
        inputs, targets = zc_imgs.to(device), zc_labels.to(device)
    elif dataload == 'random':
        ...
        try:
            for measure_name in measure_names:
                if measure_name not in measure_values:
                    ...
                    # print('time for proxy: {:.9f} s'.format(time.time() - t)) # debug
...
def find_measures(net_orig,                  # neural network
                  ...
                  zc_imgs=None,              # images as input of zero cost
                  zc_labels=None,            # labels of zc_imgs
                  teacher_output=None,       # teacher outputs used in KD based proxy
    ...
    if measures_arr is None:
        measures_arr = find_measures_arrays(net_orig, dataloader, dataload_info, device, 
                                            zc_imgs=zc_imgs, zc_labels=zc_labels, teacher_output=teacher_output, loss_fn=loss_fn, measure_names=measure_names)

    measures = {}
    for k, v in measures_arr.items():
        if k == 'jacob_cov' or k == 'kd_kl_divergence' or k == 'NASWOT' or k == 'te_nas':
            measures[k] = v
        else:
