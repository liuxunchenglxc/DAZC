...
def get_args_parser():
    ...
    parser.add_argument('--data-path', default='./data/imagenet/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET', choices=['CIFAR10', 'CIFAR100', 'CARS', 'PETS', 'FLOWERS', 'IMNET', 'INAT', 'INAT19'],
...
def main(args):
    ...
    print('number of SuperNet params:', n_parameters)
    ...
    if args.resume:
        ...
        if args.transfer:
            pre_dict = {k: v for k, v in checkpoint['model'].items() if model_without_ddp.state_dict()[k].numel() == v.numel()}
            model_without_ddp.load_state_dict(pre_dict, strict=False)
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            SAVE_ALL_EPOCH = True
            if SAVE_ALL_EPOCH:
                checkpoint_paths = [output_dir / 'checkpoint.pth', output_dir / f'checkpoint-{epoch}.pth']
            else:
